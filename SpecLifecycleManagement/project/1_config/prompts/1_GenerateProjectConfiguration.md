...
4. **Classify before rewriting.** Pass 1 labels every paragraph — or every comment — as one of:
   ...
   Write each label, with its unit reference, into the structured classification record described above as the pass proceeds. Only then rewrite. Without this forcing pass, the agent jumps straight to synthesis and that is where loss occurs.
5. **Verify coverage mechanically, do not trust the claim.** Exhaustive classification is self-reported, and the agent that skims the middle of a long document is the same agent that writes the classification of the middle of that document — so a self-reported exhaustive pass can be uniformly plausible and still have a hole. Run the coverage-count script against the classification record: classifiable units in, classification entries out, counts must match. Without this check the exhaustiveness requirement produces the *appearance* of coverage evidence, which is worse than no evidence because it gets trusted.
...
- the settled positions listed above, presented as decisions rather than open questions — including the regenerability test for redundant comments, the removed-line cap that bounds its application and the fact that the cap is universal rather than comment-specific, the divergence-escalation rule, the resolution of the `not verifiable` bucket for code comments, the retrieval-exclusion-before-deletion sequence with its full channel list, exclusion's second use as a way to retain without retrieving, the `last-verified-at` baseline resolution, and the archiving script's ownership of the archived-plan inventory;
- the rules-vs-skills split with the assignment criterion, the targeted scope of the justification requirement against the `CLAUDE.md` line budget, how the taxonomy is compressed to fit that budget, and the escalation intake as a precondition for the flag-rather-than-rewrite rule;
...
- the operations that belong to neither rules nor skills, including the mechanical verification scripts (coverage count, removed-line cap), the intake-append script, and the structured classification record that gives the coverage check something to count;
- the consolidation procedure, step by step, including commit isolation, the verification baseline and why its ordering is fixed, the structured classification record, the mechanical coverage check, the ephemeral-classification rationale, the commit-message destination and the retrieval-excluded fallback for oversized records;
- correct and incorrect triggers;
- the three-tier verification asymmetry, as a table, terminating in the intake;
- the critical positions, kept sharp, including the requirement that controls be mechanical rather than self-reported, and the bounded return on comment consolidation relative to document exclusion;
- an explicit **open questions** section for what remains genuinely undecided: target-set selection heuristics via the knowledge graph; how to measure whether consolidation is actually reducing context cost, and specifically how to confirm or falsify the prediction that comment debt is the binding constraint; how to detect contradiction across documents rather than within one; where the boundary sits between an aggressively removed document and lost institutional knowledge; the concrete form of the escalation intake and what process consumes it, given that the workflow has no defined intake phase; the numeric calibration of the removed-line cap and whether it should differ between prose and comment passes; how retrieval exclusion is implemented in a given toolchain and therefore what it actually costs; whether severing inbound references is itself safely agent-executable or must be human-reviewed like any other destructive edit; what unit counts as a "classifiable unit" for the coverage check in documents without clean paragraph boundaries; the format and path of the structured classification record, and the size threshold at which a retrieval-excluded committed record beats a commit message body; whether an unverifiable code comment being invisible in the file it lives in is an acceptable cost or requires a carrier this design currently forbids; whether comment passes need any persistent verification marker at all, given that `last-verified-at` has no carrier in source files and comment debt therefore cannot be prioritized by staleness.
...
- Do not invent facts, metrics, tool names, or file paths not present in this prompt. Where a detail is unknown — including any numeric threshold, such as the removed-line cap, and any format or path, such as that of the classification record — mark it as an open question rather than filling it in.
